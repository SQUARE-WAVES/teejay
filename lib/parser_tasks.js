var parse_gen = require("./parser.js").parse;
...
const parser_tasks = {
  ...
  "deal_with_results": function (results, gen, tag_function, next) {
    ...
    if (done) {
      ...
      next(null, "build_tagval", tag_val.tag, tag_val.val, gen, tag_function)
    }
  },

  "build_tagval": function (tag, val, gen, tag_function, next) {
    tag_function(tag, val, function (err, res) {
      if (err) {
        next(err)
      }
      else {
